...
Для упрощения создания коммитов используй **Commitizen**:

```bash
# Вместо git commit используй
bun run commit
...
Commitizen проведет через интерактивный процесс создания коммита в правильном формате.
...
```bash
# Полная проверка всех файлов и зависимостей
bun run knip
...
- ❌ Неиспользуемые зависимости (dependencies & devDependencies)
- ❌ Неиспользуемые файлы
- ❌ Неиспользуемые экспорты
- ❌ Неуказанные зависимости (unlisted)
- ✅ Дублирование кода

### Конфигурация

Конфигурация находится в `knip.json`. Некоторые зависимости специально игнорируются:
- `conventional-changelog-conventionalcommits` - используется semantic-release
- `postcss`, `tailwindcss` - используются через конфиг
- `tw-animate-css` - TailwindCSS плагин

### Когда запускать

- Перед созданием PR
- После добавления/удаления зависимостей
- Периодически для поддержания чистоты проекта
